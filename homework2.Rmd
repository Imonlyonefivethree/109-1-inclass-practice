...
```{r ans11}
# newPromotion1 
level_categiry<- c("新登場","推薦套餐","豪華雙饗丼","GYUDON","RICE AND CURRY","豬肉丼類")

newPromotion1 <- list(
  category=factor("新登場",level=level_categiry),
  item="番茄牛丼",
  description="新鮮番茄的酸味，讓人食慾大開!",
  price="109"
)
as.factor(newPromotion1$category)
as.character(newPromotion1$item,newPromotion1$description)
as.numeric(newPromotion1$price)
str(newPromotion1)
newPromotion1
...
# recommend1
recommend1<-list(
  category=factor("推薦套餐",level=level_categiry),
  item="Gyudon with Green Onion and Raw Egg",
  description="蔥溫玉牛丼+自選套餐",
  price="159"
)
as.factor(recommend1$category)
as.character(recommend1$item,newPromotion1$description)
as.numeric(recommend1$price)
str(recommend1)
recommend1
...
newPromotion1[c(1,4)]<-list("牛丼類","120")
sukiyaMenu2 <-newPromotion1
str(sukiyaMenu2)
sukiyaMenu2
```
...
```{r ans15}
# sukiyaMenu3
sukiyaMenu3 <- purrr::transpose(sukiyaMenu2)
str(sukiyaMenu3)
